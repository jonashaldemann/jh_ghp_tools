...
def apply_box_mapping(
    ...
    for geom, guid in zip(geometries, guids):
        ...
        if isinstance(geom, rg.Brep):
            print("is brep")
            face_areas = [rg.AreaMassProperties.Compute(face).Area for face in geom.Faces]
        ...
        # Für Meshes
            ...
            pts = [
            rg.Point3d(geom.Vertices[face.A].X, geom.Vertices[face.A].Y, geom.Vertices[face.A].Z),
            rg.Point3d(geom.Vertices[face.B].X, geom.Vertices[face.B].Y, geom.Vertices[face.B].Z),
            rg.Point3d(geom.Vertices[face.C].X, geom.Vertices[face.C].Y, geom.Vertices[face.C].Z),
            ]
            boxes.append(pts[0])
            boxes.append(pts[1])
            boxes.append(pts[2])
        ...
        if update:
            ...
            if obj_ref:
                mapping = TextureMapping.CreateBoxMapping(plane, x_interval, y_interval, z_interval, True)
                mapping_channel = 1
                Rhino.RhinoDoc.ActiveDoc.Objects.ModifyTextureMapping(obj_ref.Id, mapping_channel, mapping)
